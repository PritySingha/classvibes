...
$stmt = $conn->prepare("SELECT id, full_name, email, avatar, created_at, password FROM users WHERE id = ? AND role = 'teacher'");
$stmt->bind_param("i", $userId);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows === 0) {
    // Not found or not a teacher
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['change_password'])) {
    ...
    if (empty($errors)) {
        // Update database
        $stmtUpdate = $conn->prepare("UPDATE users SET full_name = ?, email = ?, avatar = ? WHERE id = ? AND role = 'teacher'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
    ...
    if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
        ...
        $stmtPass = $conn->prepare("UPDATE users SET password = ? WHERE id = ? AND role = 'teacher'");
...
<head>
    <meta charset="UTF-8" />
    <title>Teacher Profile - ClassVibes</title>
